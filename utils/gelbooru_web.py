...
def get_gelbooru_pic_list(my_config: dict, page: int):
    """
    解码文件名
    :param my_config: 配置文件
    :param page: 页码
    ...
    if include_tags:
        url = 'https://gelbooru.com/index.php?page=dapi&s=post&q=index&json=1&tags={}&pid={}'.format(include_tags, str(page))
    else:
        url = 'https://gelbooru.com/index.php?page=dapi&s=post&q=index&json=1&pid=' + str(page)  # JSON API
...
def gelbooru_download(post, my_config):
    folder_path = my_config['save_path']
    # 获取文件名并解码
    # 没错我就是嵌套狂魔
    file_name = files.rename('gelbooru ' + str(post['id']) + ' ' + str(post['image']))
    # 文件是否已存在？
    # 提醒：存在已知问题
    ...
    if files.id_exists(file_name):
        ...
        time.sleep(1)
        return True
    print('{} 开始下载 {}'.format(time.strftime('%H:%M:%S'), post['id']))
    start_time = time.time()
    img = get(post['file_url'], my_config, {
        'Host': 'img3.gelbooru.com',
        'Referer': 'https://gelbooru.com/index.php?page=post&s=view&id=' + str(post['id'])})
    cost_time = time.time() - start_time
    print('{} 下载完毕，耗时 {}s'.format(post['id'], "%.2f" % cost_time))
...
def gelbooru_worker(my_config: dict):
    """
    gelbooru.re 的爬虫函数
    :param my_config: 配置文件
    :return: None
    ...
    while pic_list_exist:
        # 通过API获取图片列表
        pic_list = get_gelbooru_pic_list(my_config, page)
        page += 1
        # 判断是否到达指定的最后一页
        if page > int(my_config['end_pages']):
            pic_list_exist = 0
        # 解析json
        pic_list = json.loads(pic_list)['post']
        # 判断是否为空页，便于提前结束进程
        if not len(pic_list):
            pic_list_exist = 0
            print('No more image!')
        while pic_list:
            post = pic_list.pop(0)
            if gelbooru_tag_judge(post, my_config):
                gelbooru_download(post, my_config)
