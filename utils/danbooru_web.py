...
def get_danbooru_pic_list(my_config: dict, page: int):
    """
    解码文件名
    :param my_config: 配置文件
    :param page: 页码
    ...
    if include_tags:
        url = 'https://danbooru.donmai.us/posts.json?tags={}&page={}&json=1'.format(include_tags, str(page))
    else:
        url = 'https://danbooru.donmai.us/posts.json?page=' + str(page)  # JSON API
...
def danbooru_tag_judge(post, my_config):
    """
    判断待排除的tag
    :param post:
    :param my_config:
    ...
    if list(set(exclude_tags).intersection(set(post['tag_string'].strip(' ').split(' ')))):
        print(post['id'], ' has exclude tags!')
        return False
    return True


def danbooru_download(post, my_config):
    folder_path = my_config['save_path']
    # 获取文件名并解码
    # 没错我就是嵌套狂魔
    file_name = files.rename('danbooru ' + str(post['id'])
                             + ' ' + str(post['tag_string_copyright'])
                             + ' ' + str(post['tag_string_character'])
                             + ' ' + str(post['tag_string_artist']))
    # 文件是否已存在？
    # 提醒：存在已知问题
    ...
    img = get(post['file_url'], my_config, {'Referer': 'https://danbooru.com/'})
...
def danbooru_worker(my_config: dict):
    """
    爬虫主函数
    :param my_config: 配置文件
    :return: None
    ...
    while pic_list_exist:
        # 通过API获取图片列表
        pic_list = get_danbooru_pic_list(my_config, page)
        page += 1
        # 判断是否到达指定的最后一页
        ...
        while pic_list:
            post = pic_list.pop(0)
            if danbooru_tag_judge(post, my_config):
                danbooru_download(post, my_config)
